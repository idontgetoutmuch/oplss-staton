...
testGmm :: IO [(Int, Product Double)]
testGmm = do
  samples <- mh gmm
  return samples

reallyTestGmm :: IO [Int]
reallyTestGmm = do
  samples <- testGmm
  return $ map fst $ take 100000 samples
...
main :: IO ()
main = (encode <$> foldMap encodeRecord <$> map Only <$> reallyTestGmm) >>=
        BL.writeFile "testGmm.csv"
